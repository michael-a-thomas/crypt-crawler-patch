...
/* ── Sticky shell ────────────────────────────────────────────────────────── */
.sticky-shell { position: sticky; top: 0; z-index: 200; }
...
.btn {
  ...
  cursor: pointer; transition: all .15s; white-space: nowrap; text-transform: uppercase;
}
.btn:active:not(:disabled) { transform: scale(.96); }
.btn:disabled              { opacity: .3; cursor: not-allowed; }
.btn--start { background: var(--accent); color: #000; }
.btn--start:hover:not(:disabled) { background: #00ffe0; box-shadow: 0 0 18px rgba(0,212,170,.28); }
.btn--stop  { background: transparent; color: var(--red); border-color: rgba(255,85,85,.35); }
.btn--stop:hover:not(:disabled)  { background: rgba(255,85,85,.1); border-color: var(--red); }
.btn--clear { background: transparent; color: var(--muted); border-color: var(--border); }
.btn--clear:hover:not(:disabled) { color: var(--red); border-color: rgba(255,85,85,.35); }

/* ── Progress panel ──────────────────────────────────────────────────────── */
.progress {
  background: var(--surface); border-bottom: 1px solid var(--border);
  padding: 7px 24px; transition: background .4s, border-bottom-color .4s;
}
.progress.is-running  { background: rgba(0,212,170,.035);  border-bottom-color: rgba(0,212,170,.25); }
.progress.is-done     { background: rgba(77,166,255,.03);  border-bottom-color: rgba(77,166,255,.2); }
.progress.is-stopped  { background: rgba(255,85,85,.03);   border-bottom-color: rgba(255,85,85,.2); }
.prg-row { display: flex; align-items: center; flex-wrap: wrap; gap: 8px 22px; }

.status-badge {
  display: inline-flex; align-items: center; gap: 5px;
  padding: 2px 10px; border-radius: 999px;
  font-size: .63rem; font-weight: 800; letter-spacing: 1.6px;
...
.sb--running   { background: rgba(0,212,170,.14); color: var(--accent); animation: statusPulse 2s ease-in-out infinite; }
...
@keyframes statusPulse { 0%,100%{opacity:1} 50%{opacity:.65} }

.prg-stat { font-size: .73rem; color: var(--muted); }
.prg-stat strong { color: var(--text); font-weight: 600; }
.prg-bar-wrap {
  flex: 1; min-width: 60px; max-width: 150px;
  height: 3px; background: var(--border); border-radius: 2px; overflow: hidden;
}
.prg-bar {
  height: 100%; background: linear-gradient(90deg, var(--accent), #00ffe0);
...
.filters {
  ...
  padding: 10px 24px; display: flex; align-items: center; flex-wrap: wrap; gap: 10px 24px;
}
...
/* ── Source chips ────────────────────────────────────────────────────────── */
.src-chips { display: flex; gap: 5px; flex-wrap: wrap; }
.src-chip {
...
.size-sel {
  ...
  padding: 4px 11px; border-radius: 6px; font-size: .72rem;
  cursor: pointer; outline: none; transition: border-color .15s; min-width: 110px;
}
.size-sel:focus { border-color: var(--accent); }

/* ── Sort buttons ────────────────────────────────────────────────────────── */
.sort-group { display: flex; gap: 5px; margin-left: auto; }
.sort-btn {
...
.sort-btn.active { background: var(--accent-lo); border-color: rgba(0,212,170,.4); color: var(--accent); }
...
.card:hover { transform: translateY(-3px); border-color: var(--border-h); box-shadow: 0 10px 30px rgba(0,0,0,.45); }
...
.card__src-badge {
  ...
  padding: 3px 8px; border-radius: 4px; color: #fff; z-index: 2;
...
.card__price-badge {
  ...
  padding: 3px 10px; border-radius: 6px; z-index: 2; letter-spacing: .3px;
}

.card__body { padding: 13px 14px 14px; flex: 1; display: flex; flex-direction: column; gap: 7px; }
.card__title {
  font-size: .84rem; font-weight: 600; line-height: 1.38; color: var(--text);
  overflow: hidden; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical;
}
.card__size {
  display: inline-block; font-size: .65rem; font-weight: 600;
  padding: 2px 8px; border: 1px solid var(--border-h); border-radius: 4px;
  color: var(--muted); width: fit-content;
}
.card__desc {
  font-size: .74rem; color: var(--muted); line-height: 1.45; flex: 1;
  overflow: hidden; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical;
}
.card__link {
  ...
  margin-top: auto; transition: all .15s; letter-spacing: .4px;
}
.card__link:hover { background: var(--accent-lo); border-color: rgba(0,212,170,.4); color: var(--accent); }

/* ── Empty / Loading ─────────────────────────────────────────────────────── */
.empty { grid-column: 1/-1; text-align: center; padding: 90px 20px; color: var(--muted); }
...
.spin-wrap { grid-column: 1/-1; display: flex; align-items: center; justify-content: center; padding: 90px 20px; }
.spinner {
...
.toast--ok  { background: rgba(0,212,170,.1); border: 1px solid rgba(0,212,170,.4); color: var(--accent); }
.toast--err { background: rgba(255,85,85,.1); border: 1px solid rgba(255,85,85,.4); color: var(--red); }
...
/* ── Responsive ──────────────────────────────────────────────────────────── */
@media (max-width: 900px) { .grid { grid-template-columns: repeat(auto-fill, minmax(200px,1fr)); } }
@media (max-width: 680px) {
  .header,.progress,.filters { padding-left: 16px; padding-right: 16px; }
...
<main class="main">
  <div class="grid" id="listings-grid">
    <div class="empty">
      ...
      <p class="empty__sub">Click <strong>Start Crawl</strong> to begin.</p>
    </div>
  </div>
</main>
...
const App = (() => {
  ...
  function init() {
    ...
    document.querySelectorAll('.src-chip').forEach(chip => {
      chip.addEventListener('click', () => {
        ...
        filters.sources = Array.from(document.querySelectorAll('.src-chip.active')).map(c => c.dataset.src);
        resetSort(); loadListings();
      });
    });
  ...
  async function loadListings() {
    ...
    try {
      ...
      grid.innerHTML = `<div class="empty"><div class="empty__icon">⚠️</div><p class="empty__title">Failed to load</p><p class="empty__sub">Check server is running on port 8787.</p></div>`;
    } finally {
      loading = false;
    }
  }
  ...
  function renderGrid(listings) {
    const grid = $('listings-grid');
    if (!listings.length) {
      grid.innerHTML = `<div class="empty"><div class="empty__icon">👟</div><p class="empty__title">No listings found</p><p class="empty__sub">Try adjusting filters or start a new crawl.</p></div>`;
      return;
    }
  ...
  function buildCard(l) {
    ...
    const srcLabel = l.source ? esc(l.source.charAt(0).toUpperCase() + l.source.slice(1).toLowerCase()) : '';
    const priceStr = (l.price != null && l.price !== '') ? '$' + parseFloat(l.price).toFixed(2) : null;
    const descStr  = l.description ? esc(l.description.substring(0,120)) + (l.description.length > 120 ? '…' : '') : '';
    const imgHtml  = l.image_url
      ? `<img class="card__img" src="${esc(l.image_url)}" alt="${esc(l.title)}" loading="lazy" onerror="this.parentNode.innerHTML='<div class=\\'card__no-img\\'><span class=\\'card__no-img-icon\\'>👟</span><span class=\\'card__no-img-txt\\'>No Image</span></div>'">`
      : `<div class="card__no-img"><span class="card__no-img-icon">👟</span><span class="card__no-img-txt">No Image</span></div>`;
    return `<div class="card">
      ...
      <div class="card__body">
        ...
        <a class="card__link" href="${esc(l.url)}" target="_blank" rel="noopener noreferrer">View on ${srcLabel || 'Marketplace'} ↗</a>
      </div>
    </div>`;
  }
  ...
  async function startCrawl() {
    ...
    } catch (_) { toast('Network error', 'err'); $('btn-start').disabled = false; }
  }
  ...
  async function clearAll() {
    ...
    try {
      ...
      if (data.success) {
        ...
        resetSort(); loadListings(); loadFilters();
      } else { toast('Failed to clear', 'err'); }
    } catch (_) { toast('Network error', 'err'); }
    finally { $('btn-clear').disabled = false; }
  }

  // ── Status polling ────────────────────────────────────────────────────────
  // loadListings() fires ONLY when status transitions running→completed/stopped.
  // Every other poll tick is a cheap status-only fetch — no grid re-render.
  ...
  function renderProgress(s, currentStatus) {
    const labels = { idle:'Idle', running:'Running', stopping:'Stopping', completed:'Done', stopped:'Stopped' };
    ...
    show('prg-source-wrap', active); show('prg-term-wrap',   active);
    show('prg-method-wrap', active); show('prg-count-wrap',  active);
    show('prg-dupes-wrap',  active); show('prg-rej-wrap',    active);
    ...
    if (active) {
      ...
      const pct = s.target > 0 ? Math.min(100, Math.round((s.accepted / s.target) * 100)) : 0;
